...
class Tsr extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->load->model('billpayment_model', 'billpayment_model', true);
    ...
    public function main($id_campaign = '')
    {
        ...
        if (!empty($id_prospect)) {
            ...
            if ($is_priority['is_priority'] == 1) {
                ...
                $prospect = $this->prospect_model->get_list_prospect(" id_prospect='$id_prospect' AND id_tsr='$id_user' AND tp.`is_priority`=1");
                // } else {
                //getcustomer
            ...
            if ($camps['multiproduct'] != '1') :
                ...
                } elseif ($camps['campaign_product'] == 60) {
                    $this->load->model('admin/msc_model');
                    ## Get Refresh
                    $this->load->model('cop_model');
            ...
            elseif ($camps['multiproduct'] == '1') :
                ...
                foreach ($product_xsell as $xsell) {
                    ...
                    if ($xsell == 'COP') {
                        ...
                        }else if ($main_product == '57') {
                            ## Get 
                            $this->load->model('cop_model');
                            $data['refreshbagcop'] = $this->cop_model->get_datarefresh($data["prospect"]['cif_no'], $data["prospect"]['card_number_basic'], $data["prospect"]['refreshcode']);
                            ## Get Installment Tree
                            if (strpos($prospect[0]['name'], 'NTK') > 0) {
                                $data['list_productcode_cop'] = $this->cop_model->get_productcode_bytype('COP', "(status='{$prospect[0]['status']}' OR status='TYPE 28')");
                            } else if (strpos($prospect[0]['name'], 'DAP') > 0) {
            ...
            /*if($camps['campaign_product'] == 46 && $camps['multiproduct'] == 1 ){
                ## Get Refresh
                $this->load->model('cop_model');
                $data['refreshbag'] = $this->cop_model->get_datarefresh($data["prospect"]['cif_no'], $data["prospect"]['card_number_basic'], $data["prospect"]['refreshcode']);
            ...
            } else if($camps['campaign_product'] == 44 && $camps['multiproduct'] == 1 ){ 
                $data['refreshbag'] = array();
                $data['list_productcode'] = array();
                ...
                $data_xsell = json_decode($rowoff['xsell_cardxsell']); 
                if(in_array( "COP", $data_xsell) == 'true'):
                    ## Get Refresh
                    $this->load->model('cop_model');
                    $data['refreshbag'] = $this->cop_model->get_datarefresh($data["prospect"]['cif_no'], $data["prospect"]['card_number_basic'], $data["prospect"]['refreshcode']);
...
*/
    ...
    public function list_prospect($id_campaign = "", $keyword = "null", $id_callcode = '0', $page = "", $pageValue = "")
    {
        ...
        if ($id_callcode_search != 'null' && $id_callcode_search != '' && $id_callcode_search != '0' && $id_callcode_search != '-1') {
            $this->db->where('parent_id_callcode', $id_callcode_search);
            $this->db->select('GROUP_CONCAT(id_callcode) AS childcode', FALSE);
            $this->db->where_not_in('id_callcode', $noting); //SCRIPT BARU
            $qObj = $this->db->get('tb_callcode');
            if ($qObj->num_rows() > 0) {
                $qArr = $qObj->row_array();
                $childcode = $qArr['childcode'];
        ...
        //var_dump($childcode);
        if ($id_callcode == '0' || $id_callcode == '-1') {
            $extra_cond_1 = "tp.last_id_callcode IN ($childcode) AND is_recycle = 0"; // INI buat NEW DATA
            $extra_cond_2 = "tp.last_id_callcode > 0 AND is_recycle = 1"; // ini buat data reload

            $condition .= $id_callcode_search == 'null' || $id_callcode_search == '' ? " "
                : ' AND is_agree!=1 AND ((' . $extra_cond_1 . ') OR (' . $extra_cond_2 . '))';
        } else if ($id_callcode != 52) {
            $condition .= $id_callcode_search == 'null' || $id_callcode_search == '' ? " "
                : ' AND is_agree!=1 AND tp.last_id_callcode IN(' . $childcode . ') AND is_recycle = 0';
        }
        ...
        ## Recycle Flag
        //        if($id_callcode_search == '-1' || $id_callcode_search == "0" ){
        //            $condition .= ' AND is_recycle >= 0';
        //        } else if($id_callcode_search == 'null' || $id_callcode_search == '') {
        //            $condition .= ' ';
        //        } else {
        //            $condition .= ' ';
    ...
    { //AGREE PL
        ...
        ## Check Campaign Type
        $this->load->model('misc/misc_model');
        $xsellbuytype = $this->input->post('typexsell', TRUE);
        $xsellidx = $this->input->post('xsellidx', TRUE);
        $xselldata = $this->misc_model->get_tableDataById('tb_xsell', $xsellidx, 'idx');

        if ($xselldata) {
            $xsell_offer = JSON_DECODE($xselldata['xsell_cardxsell'], true);
            $key = array_search($xsellbuytype, $xsell_offer);
            unset($xsell_offer[$key]);
            $upd = array(
                'xsell_cardxsell' => json_encode($xsell_offer)
            );
            $this->db->where('idx', $xsellidx);
            $this->db->update('tb_xsell', $upd);
        }
    ...
    function agree_uplan($id_prospect, $id_product, $type = "")
    { // Agree UPLAN
        $id_calltrack = $this->input->post('id_calltrack', TRUE);
 ...
 */
    ...
    public function submit_calltrack($parent_id = '', $id_prospect = '')
    {
        ...
        */
        ...
        ## Validasi XSELL Status Sudah Agree Tidak Berubah
        $this->load->model('misc/misc_model');
        $get_lastIdCallcode = $this->misc_model->get_tableDataById('tb_calltrack', $id_prospect, 'id_prospect');

        $get_isagree = $this->misc_model->get_tableDataById('tb_prospect', $id_prospect, 'id_prospect');

        //var_dump($get_lastIdCallcode);die();

        if ($get_isagree['last_id_callcode'] == '52') {
            ## Add to Agree
            $calltrack = array(
                'id_notelp' => $get_lastIdCallcode['id_notelp'],
                'id_user' => $this->id_user,
                'id_spv' => $get_lastIdCallcode['id_spv'],
                'id_tsm' => $get_lastIdCallcode['id_tsm'],
                'username' => $get_lastIdCallcode['username'],
                'id_callcode' => $get_lastIdCallcode['id_callcode'],
                'id_prospect' => $get_lastIdCallcode['id_prospect'],
                'no_contacted' => $get_lastIdCallcode['no_contacted'],
                'seq' => $get_lastIdCallcode['seq'],
                'call_date' => $datecall,
                'call_time' => $timecall,
                'call_month' => $month,
                'call_attempt' => $get_lastIdCallcode['call_attempt'],
                'sip_call' => $unix,
                'remark' => $remark,
                'id_campaign' => $id_campaign,
                'id_product' => $product_id,
                'begin_call_time' => $begin_call_time,
                'end_call_time' => $end_call_time,
                'total_call_time' => $total_call_time,
                'last_datespv' => $get_lastIdCallcode['last_datespv'],
                'last_datetsr' => $get_lastIdCallcode['last_datetsr'],
                'last_isrecycle' => $get_lastIdCallcode['last_isrecycle'],
                'last_recycledate' => $get_lastIdCallcode['last_recycledate'],
                'last_idcallcode' => $get_lastIdCallcode['last_idcallcode'],
                'id_line_call' => $id_line_call,
                'context' => $context
            );
            $this->db->insert('tb_calltrack', $calltrack);
        } else {
        ...
        $this->load->model('misc/misc_model');
        $get_prospect = $this->misc_model->get_tableDataById('tb_prospect', $id_prospect, 'id_prospect');
        if ($get_prospect['is_priority'] == 2) :
            $this->set_accountcode($id_prospect, $product_id);
        endif;
        ...
        ## Validasi Data Agree Tidak Ikut Berubah
        if ($get_prospect['last_id_callcode'] != '52') {
    ...
    public function sip_call()
    {
        ...
        } else if ($is_priority == 2 && $is_agree != 1) {
            $callblock_enable = 0; ## Overide to disable call blocking
        }

        if ($callblock_enable == 1) :

            ## check allowed to do call or not cyrusenyx2 
            $this->load->model('calllimit_model');
            $call_allowed = $this->calllimit_model->is_allowedcall($prospectArr);
            //var_dump($call_allowed);
            if ($call_allowed['flag'] == 1) {
                ## kalau tidak ada tanggal lock call cek jatah telp berturut2nya
                $in_calllimit = $this->calllimit_model->is_in_calllimit($prospectArr);

                if ($in_calllimit['flag'] == 0) {
                    //var_dump($in_calllimit['message']);
                    echo '<script  type="text/javascript">';
                    echo "sipCallWarning('" . $in_calllimit['message'] . "')";
                    echo '</script>';
                    die();
                } else {
                    echo '<script  type="text/javascript">';
                ...
                echo '<script  type="text/javascript">';
                echo "sipCallWarning('" . $call_allowed['message'] . "')";
                echo '</script>';
                die();
            }
    ...
    public function set_waitingktp()
    {
        $this->load->helper('cyrusenyx');
        $this->load->model('misc/misc_model');
        ...
        $data = $this->data_call_first($prospectArr);
        $data['remark'] = 'Waiting KTP';
        //$data['id_callcode'] = '52';
        $waitktp['id_callcode'] = '184';
        if (isset($id_prospect)) {
            $insert_id = $this->notelp_model->insert_sip_call($data);

            ## Update Table Prospect ( last update )
            $update = array(
                'waiting_ktp' => $waitktp['id_callcode'],
                'last_remark' => '',
                'last_calltime' => $now
            );
            $this->db->where('id_prospect', $id_prospect);
        ...
        if ($parent_id == 52) { //CALLTRACK AGREE
            if ($multioffers == '0') { ## Main Product
                if ($id_product == 31) { // AGREE CC
                    ...
                    $merge['submit1_url'] = site_url() . 'tsr/agree_v3/' . $id_prospect . '/' . $id_product;
                    $merge['prospect_detail'] = $this->prospect_model->get_prospectdetail($id_prospect);
                    $merge['productcode'] = $this->product_model->get_productcode('PL', "status='{$row2['status']}'");
                    ...
                    $this->load->model('offer_model');
                    $this->load->model('tsr/cardproduct_model', 'cardproduct');
                    $merge['submit1_url'] = site_url() . 'tsr/agree_ls/' . $id_prospect . '/' . $id_product;
                    $merge['prospect_detail'] = $this->prospect_model->get_prospectdetail($id_prospect);
                ...
                } else if ($id_product == 59 || $multioffers == 'CPILX') { // LiveSmart
                    $this->load->model('offer_model');
                    $this->load->model('tsr/cardproduct_model', 'cardproduct');
                ...
                } else if ($id_product == 55 || $multioffers == 'UPLAN') { // CASA
                    $this->load->model('tsr/cardproduct_model', 'cardproduct');
                    $merge['submit1_url'] = site_url() . 'tsr/agree_uplan/' . $id_prospect . '/' . $id_product;
                    //$merge['submit1_url'] = site_url().'agree_49/submit_agree/'.$id_prospect.'/'.$id_product;
                    $merge['prospect_detail'] = $this->prospect_model->get_prospectdetail($id_prospect);
                    $merge['cardsegment'] = $this->cardproduct->getlist_cardsegment_ntb();
                    $merge['productcode'] = $this->product_model->get_productcode('UPLAN');
                    $merge['list_branchcode'] = $this->product_model->getlist_branchcode();
                    $merge['list_bank'] = $this->product_model->get_bankname();
                    $merge['list_paymentsource'] = $this->product_model->get_paymentsource('is_active = 1');
                    $merge['id_product'] = $id_product;
                    ...
                    $merge['multiproduct'] = $row1['multiproduct'];
                    $merge['max_tertanggung'] = $row1['max_tertanggung'];
                    $this->load->view('tsr/agree_ntuplan', $merge);
                } else if ($id_product == 58 || $multioffers == 'SUPFR') { // Supplementary Friction
                    $this->load->model('misc/misc_model');
                    $merge['miscModel'] = $this->misc_model;
                    //$id_prospect = $this->uri->segment(4);
                    //die('aa');
                    $merge['submit1_url'] = site_url() . 'agree_58/submit_agree/' . $id_prospect . '/' . $id_product;
                    //var_dump($merge['submit1_url']);die();
                    $this->load->model('offer_model');
                    $merge['prospect_detail'] = $this->prospect_model->get_prospectdetail($id_prospect);
                    $merge['list_bank'] = $this->product_model->get_bankname();
                    $merge["xsellidx"] = $this->offer_model->get_offer_byidx($merge['prospect_detail']['cif_no'], $data_campaign_id, $merge['prospect_detail']['card_number_basic'], $id_prospect);
                    $merge['productcode'] = $this->product_model->get_productcode('SUPFR');
                    $merge['id_product'] = $id_product;
                    $merge['no_contacted'] = $no_contacted;
                    $merge['id_calltrack'] = $id_calltrack;
                    $merge['list_kurir'] = $kurirArr;
                    ...
                    $merge['multioffers'] = $multioffers;
                    $merge['multioffcard'] = $multioffcard;
                    $merge['max_tertanggung'] = 1;
                    //var_dump($merge['multioffcard']);die();
                    $merge['max_tertanggung'] = $row1['max_tertanggung'];
                    $this->load->view('tsr/agree_supfr', $merge);
                } else if ($id_product == 36 || $multioffers == 'BP') { // BILLPAYMENT
                    $this->load->model('misc/misc_model');
                    $this->load->model('billpayment_model');
                    $id_campaign = $this->misc_model->get_tableDataById('tb_prospect', $id_prospect, 'id_prospect', 'id_campaign');
                    $cif_no = $this->misc_model->get_tableDataById('tb_prospect', $id_prospect, 'id_prospect', 'cif_no');
                    //$id_prospect = $this->uri->segment(4);
                    ...
                    $merge['list_cards'] = $this->billpayment_model->get_cust_cards($cif_no, 'is_active = 1', $id_campaign);
                    $merge['insert_id'] = "";
                    $merge['multioffers'] = $multioffers;
                    $merge['max_tertanggung'] = $row1['max_tertanggung'];
                    $this->load->view('tsr/agree_bp', $merge);
                } else if ($id_product == 57 || $multioffers == 'CPIL') { // Cash PLus
                    $this->load->model('misc/misc_model');
                    $merge['miscModel'] = $this->misc_model;
                    //$id_prospect = $this->uri->segment(4);
                ...
                } else if ($id_product == 60 || $multioffers == 'FOPST') { // FlexiPay
                    $this->load->model('transactionFp_model');
                ...
                if ($multioffers == 'CC') { // AGREE CC
                    ...
                    if ($row2['campaign_product'] == '32') {
                        $merge['productcode'] = $this->product_model->get_productcode('PL', "status='{$row2['status']}'");
                        // echo $this->db->last_query(); exit();
                    } else {
                        $merge['productcode'] = $this->product_model->get_productcode('PL', "status='{$row2['status2']}'");
                    }
                    ...
                    $merge['prospect_detail'] = $this->prospect_model->get_prospectdetail($id_prospect);
                    $merge['list_bank'] = $this->product_model->get_bankname();
                    $merge["offers"] = $this->offer_model->get_offer_byCardNum($merge['prospect_detail']['cif_no'], $data_campaign_id2, $merge['prospect_detail']['card_number_basic'], $id_prospect);
                    $merge['cardnumber'] = $this->activasi_model->get_activasi_byCardNum($merge['prospect_detail']['cif_no'], $data_campaign_id2, $merge['prospect_detail']['card_number_basic'], $id_prospect);
                    ...
                    $merge['max_tertanggung'] = $row1['max_tertanggung'];
                    $this->load->view('tsr/agree_upgrade', $merge);
                } else if ($multioffers == 'FP') { // FlexiPay
                    $this->load->model('offer_model');
                    $this->load->model('transactionFp_model');
                    $merge['submit1_url'] = site_url() . 'agree_44/submit_agree/' . $id_prospect . '/' . $id_product;
                    $merge['prospect_detail'] = $this->prospect_model->get_prospectdetail($id_prospect);

                    if ($row2['campaign_product'] == '46') {
                        $merge['productcode'] = $this->product_model->get_productcode('FP', "status='{$row2['status2']}'");
                    } elseif ($row2['campaign_product'] == '32') {
                        $merge['productcode'] = $this->product_model->get_productcode('FP', "status='{$row2['status2']}'");
                    } else {
                        $merge['productcode'] = $this->product_model->get_productcode('FP', "status='{$row2['status']}'");
                    }
                    ...
                    // $db_type = $row1['type_name'];
                    // if($db_type == 'NTK'){
                    //     $merge['productcode'] = $this->product_model->get_productcode('COP', "(status='{$row1['status']}' OR status='TYPE 28') AND `segment` NOT LIKE 'COP TMRW%'");
                    // } else if($db_type == 'DAP'){
                    //     $merge['productcode'] = $this->product_model->get_productcode('COP', "(status='DAP')");
                    // } else if($db_type == 'TMRW'){
                    //     $merge['productcode'] = $this->product_model->get_productcode('COP', "(status='{$row1['status']}' AND `segment` LIKE 'COP TMRW%')");
                    // } else {
                    //     $merge['productcode'] = $this->product_model->get_productcode('COP', "(status='{$row1['status']}' AND segment!='{$row1['status']}') AND `segment` NOT LIKE 'COP TMRW%'");
                    // }

                    if ($row2['campaign_product'] == '44') {
                        $merge['productcode'] = $this->product_model->get_productcode('COP', "status='{$row2['status2']}'");
                        if (strpos($data_campaign_name2, 'NTK') > 0) {
                            $merge['productcode'] = $this->product_model->get_productcode('COP', "(status='{$row2['status2']}' OR status='TYPE 28') AND `segment` NOT LIKE 'COP TMRW%'");
                        ...
                        if (strpos($data_campaign_name2, 'NTK') > 0) {
                            $merge['productcode'] = $this->product_model->get_productcode('COP', "(status='{$row2['status']}' OR status='TYPE 28') AND `segment` NOT LIKE 'COP TMRW%'");
                        } else if (strpos($data_campaign_name2, 'DAP') > 0) {
                            $merge['productcode'] = $this->product_model->get_productcode('COP', "(status='DAP')");
                        } else if (strpos($data_campaign_name2, 'TMRW') > 0) {
                            $merge['productcode'] = $this->product_model->get_productcode('COP', "(status='{$row2['status']}' AND `segment` LIKE 'COP TMRW%')");
    ...
    function load_formPL($id_prospect = "", $id_product = "", $id_calltrack, $no_contacted = "", $insert_id = "")
    {
        ...
        $merge['submit1_url'] = site_url() . 'tsr/agree_v3/' . $id_prospect . '/' . $id_product . '/BUNDLING/';
 ...
 */
    ...
    function last_callhistory($id_prospect = 0, $limit = 10)
    {
        ...
        $qryAgent->select("tb_calltrack.remark AS call_notes, tb_calltrack.*, tb_prospect.*, tb_callcode.*, TIME_TO_SEC(TIMEDIFF(CONCAT(tb_calltrack.call_date,' ',tb_calltrack.call_time), tb_prospect.last_recycle)) AS  recycle", FALSE);
    ...
    public function set_disconected()
    {
        $this->load->helper('cyrusenyx');
        $this->load->model('misc/misc_model');
        ...
        $data = $this->data_call_first($prospectArr);
        $data['remark'] = '';
        $data['id_callcode'] = '186'; // Disconnected
        if (isset($id_prospect)) {
            $insert_id = $this->notelp_model->insert_sip_call($data);
            ...
            $update = array(
                ...
                'is_newpriority' => 0,
                'date_priorityresponse' => DATE('Y-m-d H:i:s'),
                'is_priority' => 0, // DumpOut From Priority When Agree
            );
            $this->db->where('id_prospect', $id_prospect);
            $this->db->update('tb_prospect', $update);
            ...
            $this->set_accountcode($id_prospect, $id_product);
        }
        echo site_url() . 'tsr/main/' . $id_campaign;
        exit;
    }

    public function set_accountcode($id_prospect, $id_product)
    {
        if ($id_prospect) :
            ## Update No Case to Asterisk Server
            $this->load->model('misc/misc_model');
            $get_prospect = $this->misc_model->get_tableDataById('tb_prospect', $id_prospect, 'id_prospect');
            if ($get_prospect['is_priority'] == 2) :
                $DB2 = $this->load->database('cc', TRUE); //load asterisk db   
                $DB2->where('uniqueid', $get_prospect['inbound_uniqueid']);
                $data_update = array(
                    'accountcode' => $id_prospect,
                    'userid' => $_SESSION['id_user'],
                    'id_calltrack' => $id_calltrack,
                    'id_product' => $id_product
                );
                $DB2->update('cdr', $data_update);
            endif;
        ## End Update No Case to Asterisk Server
        endif;
    }
}
